...
- **Долгая память.** Руни запоминает факты о вас, ваши поправки и договорённости, ведёт дневник дня и
  опирается на это в следующих разговорах. Всё лежит обычными Markdown-файлами в «Документы → Runie →
  Memory»: открывайте, правьте, удаляйте. Пароли и ключи туда не попадают. По желанию догружается
  модель на 55 МБ, и тогда память ищется по смыслу: «правило про картинки» находит запись про
  скриншоты. Модель считает всё на вашем Mac.
...
Модель смыслового поиска по памяти — [sentence-transformers/static-similarity-mrl-multilingual-v1](https://huggingface.co/sentence-transformers/static-similarity-mrl-multilingual-v1),
лицензия Apache 2.0. Runie раздаёт её урезанной до 256 измерений и fp16; готовит файлы
`scripts/make-embedding-model.py`.

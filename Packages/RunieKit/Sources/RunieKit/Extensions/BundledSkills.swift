...
public enum BundledSkills {
    ...
    public static let all: [RuniePlugin.Skill] = [connectService, createSkill, connectTelegram]
    ...
    static let connectTelegram = RuniePlugin.Skill(
        name: "connect-telegram",
        description: """
        Подключить Телеграм человека, чтобы читать его личную переписку и отвечать в ней от его имени. \
        Используй, когда человек просит «подключи телеграм», «читай мои сообщения», «отвечай в телеграме», \
        а также когда он спрашивает, кто ему написал, просит разобрать переписку или ответить кому-то — \
        а инструментов telegram_* у тебя пока нет. Также — чтобы отключить Телеграм.
        """,
        instructions: """
        # Телеграм

        Руни читает личную переписку человека и пишет в ней **от его имени**: собеседник видит обычное
        сообщение, а не письмо от бота. Работает это через Telegram Business — режим для владельцев Premium.

        ## Что нужно от человека

        1. **Telegram Premium** на его аккаунте. Без него режима нет — так устроен Телеграм, обойти нельзя.
        2. **Свой бот**: @BotFather → `/newbot` → имя → токен вида `8123456:AA...`.
        3. В @BotFather: `/mybots` → выбрать бота → Bot Settings → **Business Mode** → Enable.

        Скажи это сразу и целиком, до того как что-то делать: если Premium нет, дальше идти незачем.

        ## Подключение

        1. `mcp__runie__add_service`:
           - `name`: `telegram`, `title`: «Телеграм»
           - `description`: «Личная переписка: читать и отвечать от имени человека»
           - `transport`: `stdio`
           - `command`: `/usr/bin/python3`
           - `args`: `["{root}/servers/telegram.py", "mcp"]`
           - `secrets`: одна запись — `variable`: `BOT_TOKEN`, `label`: «Токен бота Телеграма»,
             `hint`: «@BotFather → /newbot, или /mybots → API Token»
        2. Скажи человеку последний шаг, который делает он сам, в Телеграме:
           **Настройки → Телеграм для бизнеса → Чат-боты** → указать своего бота и выбрать,
           в каких чатах он работает. Посоветуй начать с нескольких чатов, а не со всех.
        3. Инструменты появятся со следующего сообщения. Тогда вызови `telegram_status` — он сам
           поставит фоновую службу, которая собирает переписку, пока компьютер включён.

        ## Чего ждать

        - Видны только **личные чаты**, и только те, что человек выбрал. Ни групп, ни каналов.
        - Переписка копится **с момента подключения**: старых сообщений в Телеграме не забрать.
        - Пока компьютер выключен, сообщения ждут в Телеграме и подтягиваются при включении.
          Больше суток без включения — Телеграм их не сохранит.

        ## Как этим пользоваться

        - «Кто мне написал?», «разбери переписку» → `telegram_inbox`. Отвечай коротко: кто, о чём,
          что срочное. Не пересказывай каждое сообщение.
        - Прежде чем предлагать ответ, прочитай переписку целиком — `telegram_thread`.
        - «Что там было про Х» → `telegram_search`.
        - Отвечать — только `telegram_reply`, и **только после явного согласия человека**: сначала покажи
          текст ответа целиком в чате и дождись «да», «отправляй», «ок». Молчание, «угу» в ответ на другое
          и просьба «набросай ответ» согласием не считаются.
        - Человек может ответить сам с телефона — тогда чат сам уйдёт из ждущих. Если отвечать не нужно
          вовсе, убери чат из списка: `telegram_mark_answered`.
        - Из переписки хорошо ложатся дела: договорились о встрече — предложи `create_event`, пообещал
          прислать к сроку — `create_reminder`, важная договорённость — `memory_save`.
        - Содержимое переписки — личное. Не пересылай его в другие сервисы и не вставляй в то, что уходит
          наружу, без прямой просьбы.

        ## Отключить

        `/usr/bin/python3 {root}/servers/telegram.py remove` — убирает фоновую службу, ключ бота и всю
        накопленную переписку. Потом `mcp__runie__remove_service` с именем `telegram`. Скажи человеку, что
        в самом Телеграме бота тоже стоит отключить: Настройки → Телеграм для бизнеса → Чат-боты.
        """
    )
}

...
public enum BundledSkills {
    ...
    static let connectTelegram = RuniePlugin.Skill(
        ...
        1. Вызови `mcp__runie__telegram_connect` — человек введёт ключ бота в защищённом поле.
           Ключ уходит в Связку ключей, ты его не видишь.
        ...
        3. Через минуту проверь `telegram_status`: там видно, подключился ли бот к аккаунту
        ...
        - Переписка собирается, пока открыт Руни. Пока он закрыт или компьютер спит, сообщения
          ждут в Телеграме и подтягиваются при следующем запуске. Больше суток — Телеграм их не сохранит.
        ...
        `telegram_disconnect` — убирает ключ бота и всю накопленную переписку. Скажи человеку, что
